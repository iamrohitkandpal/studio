...
const HtmlIcon = () => <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M13.5 6l-1.4 1.4 4.6 4.6-4.6 4.6L13.5 18l6-6zM4.5 18l1.4-1.4-4.6-4.6 4.6-4.6L4.5 6l-6 6z"/></svg>;
const CssIcon = () => <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M9.4 16.6L4.8 12l4.6-4.6L8 6l-6 6 6 6 1.4-1.4zm5.2 0l4.6-4.6-4.6-4.6L16 6l6 6-6 6-1.4-1.4z"/></svg>;
...
const SkillItem: React.FC<{ name: string; icon: React.ComponentType; level?: string }> = ({ name, icon: Icon, level }) => (
  ...
  >
    <Icon />
    {/* Skill name using body font */}
    <span className="mt-2 text-xs sm:text-sm font-body">{name}</span>
     {/* Level indicator using caption font */}
     {level && <span className="text-xs font-caption text-foreground/60">({level})</span>}
  </motion.div>
);

const SkillCategory: React.FC<SkillCategoryProps> = ({ title, skills }) => (
  <div className="mb-6">
    {/* Category title using heading font */}
    <h3 className="text-lg font-heading font-semibold mb-3 text-primary/80">{title}</h3>
