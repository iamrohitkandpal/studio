...
const Achievements: React.FC = () => {
  const achievements = [
    { text: 'Maintained a 100-day streak in Data Structures and Algorithms practice on LeetCode.', year: 2025 }, // Future year, adjust if needed
      ...
      <ul className="space-y-3">
        {achievements.map((ach, index) => (
          <li key={index} className="flex items-start gap-3 glass-card p-4 rounded-lg border border-border/30">
            ...
            <div>
                <p className="text-foreground/90">{ach.text}</p>
                <p className="text-xs text-foreground/60">({ach.year})</p>
            </div>
          </li>
        ))}
      </ul>
